...
export default function App() {
  ...
  const handleUpdateGoalProgress = (goalId: string, amount: number) => {
    ...
    if (cashWallet.balance < amount) {
      alert(`Sá»‘ dÆ° trong vÃ­ "${cashWallet.name}" khÃ´ng Ä‘á»§ (hiá»‡n cÃ³ Â₫${cashWallet.balance}) Ä‘á»ƒ chuyá»ƒn khoáº£n Ä‘Ãºt heo nhen! Báº¡n cÃ³ thá»ƒ náº¡p thÃªm tiá»n hoáº·c chá»‰nh sá»­a sá»‘ dÆ° vÃ­ trÆ°á»›c nha! ðŸ₫º`);
      return;
    }
    ...
    const savingsTx: TransactionModel = {
      id: `tx-goal-${Date.now()}`,
      amount,
      note: `Gá»­i á»‘ng heo tiáº¿t kiá»‡m: ${goals.find(g => g.id === goalId)?.name} ðŸ·ðŸŒŸ`,
    ...
    alert(`ÄÃ£ trÃ­ch Â₫${amount.toLocaleString()} tá»« vÃ­ "${cashWallet.name}" Ä‘á»ƒ Ä‘Ãºt heo tiáº¿t kiá»‡m thÃ nh cÃ´ng! Tháº­t tuyá»‡t vá»i! ðŸŽ‰ðŸŒ¸`);
  };
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden max-w-md mx-auto w-full bg-white md:shadow-xl md:border-x-4 md:border-kawaii-pink-200">
        ...
        <nav className="fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t-3 border-kawaii-pink-100 px-6 py-2 pb-5 flex justify-between items-center max-w-md mx-auto shadow-[0_-5px_15px_rgba(255,181,197,0.15)] rounded-t-[28px]">
          ...
          >
            ...
            <span className="text-[10px] font-display font-bold">Tá»•ng quan</span>
            {activeTab === 'dashboard' && (
          ...
          >
            ...
            <span className="text-[10px] font-display font-bold">PhÃ¢n tÃ­ch</span>
            {activeTab === 'analytics' && (
              <motion.span layoutId="activeDot" className="absolute -top-1 w-1.5 h-1.5 rounded-full bg-kawaii-pink-400" />
            )}
          </button>
          ...
          <div className="relative -mt-8 flex justify-center items-center">
            <button
              ...
              title="Ghi chÃ©p chi tiÃªu má»›i"
            >
          ...
          >
            ...
            <span className="text-[10px] font-display font-bold">VÃ­ tiá»n</span>
            {activeTab === 'wallets' && (
              <motion.span layoutId="activeDot" className="absolute -top-1 w-1.5 h-1.5 rounded-full bg-kawaii-pink-400" />
            )}
          </button>
