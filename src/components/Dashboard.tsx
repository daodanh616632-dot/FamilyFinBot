...
export default function Dashboard({
  ...
  const handleQuickAction = (label: string) => {
    if (label === 'ThÃªm') {
      ...
      alert(`Báº¡n Ä‘Ã£ chá»n sá»• con: ${label}! ChÃºc báº¡n cÃ³ má»™t ngÃ y trÃ n ngáº­p niá»m vui nha! ðŸ₫°ðŸŒ¸`);
    }
  };
  ...
  const handleGoalContributionSubmit = (e: React.FormEvent) => {
    ...
    if (isNaN(amount) || amount <= 0) {
      alert('Vui lÃ²ng nháº­p sá»‘ tiá»n Ä‘Ã³ng gÃ³p há»£p lá»‡ lá»›n hÆ¡n 0 nhÃ©! ðŸ’–');
      return;
    }
  ...
  const handleSaveBudget = () => {
    ...
    if (!isNaN(val) && val > 0) {
      ...
      alert('Vui lÃ²ng nháº­p giá»›i háº¡n ngÃ¢n sÃ¡ch há»£p lá»‡ nhen! âœ¨');
    }
  };

  return (
    <div className="flex-1 overflow-y-auto pb-24">
      {/* 1. Header (MÃ u há»“ng pastel bo gÃ³c dÆ°á»›i dáº¡ng SÃ³ng hoáº·c ClipPath) */}
        ...
        <div className="max-w-md mx-auto flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-display font-medium text-kawaii-pink-600 flex items-center gap-1">
                ...
                MOMO KAKEIBO â€¢ NHáº¬T KÃ CHI TIÃŠU
              </p>
              <h1 id="app-title-main" className="text-2xl font-display font-bold text-kawaii-pink-600">Sá»• cÃ¡i máº·c Ä‘á»‹nh ðŸ““</h1>
            </div>
            
            {/* Cute Avatar or Mascot */}
            <div className="w-10 h-10 rounded-full border-2 border-white bg-kawaii-pink-100 flex items-center justify-center text-xl shadow-sm hover:scale-105 transition-transform duration-200 cursor-pointer" title="CÃ¡ nhÃ¢n">
              ðŸ°
            </div>
          </div>
          ...
          <div className="relative flex items-center">
            ...
            <input
              ...
              type="text"
              placeholder="TÃ¬m kiáº¿m ghi chÃº giao dá»‹ch... ðŸ”ðŸŒ¸"
              value={searchTerm}
          ...
          <div className="flex justify-around items-center mt-3 bg-white/40 p-2.5 rounded-2xl border border-white/40 backdrop-blur-sm">
            {[
              { id: 'btn-qa-babe', icon: 'ðŸ’–', label: 'Babe' },
              { id: 'btn-qa-travel', icon: 'ðŸŽ’', label: 'Travel' },
              { id: 'btn-qa-ledger', icon: 'ðŸ““', label: 'Sá»• cÃ¡i' },
              { id: 'btn-qa-add', icon: 'âž•', label: 'ThÃªm', highlight: true },
            ].map(act => (
              <button
                id={act.id}
                key={act.label}
        ...
        <div className="kawaii-card bg-white p-5 relative overflow-hidden">
          ...
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-1 bg-kawaii-pink-50 border border-kawaii-pink-100 rounded-full px-2.5 py-0.5 text-xs text-kawaii-pink-600 font-medium">
              <Calendar size={12} />
              <span>ThÃ¡ng 7/2026</span>
            </div>
            <span className="text-[10px] font-mono text-gray-400">ThÃ¡ng nÃ y cá»§a báº¡n ðŸŒ¸</span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 divide-x divide-gray-100 mb-2">
            <div>
              <p className="text-[11px] text-gray-400 font-medium flex items-center gap-0.5">
                <TrendingUp size={11} className="text-emerald-400" /> Tá»•ng Thu Nháº­p
              </p>
            ...
            <div className="pl-4">
              <p className="text-[11px] text-gray-400 font-medium flex items-center gap-0.5">
                <TrendingDown size={11} className="text-rose-400" /> Tá»•ng Chi TiÃªu
              </p>
          ...
          <div className="border-t border-dashed border-kawaii-pink-100 pt-3 flex items-center justify-between">
            <span className="text-xs font-display font-medium text-gray-500">Sá» DÆ¯ HIá»†N Táº I:</span>
      ...
      <div className="max-w-md mx-auto px-4 mt-5 flex flex-col gap-5">
        ...
        <div className="kawaii-card p-4">
          <div className="flex items-center justify-between mb-2">
            <div>
              <h3 className="text-xs font-display font-bold text-gray-600 flex items-center gap-1.5">
                ðŸ“‹ Háº¡n Má»©c NgÃ¢n SÃ¡ch ThÃ¡ng
              </h3>
              <p className="text-[10px] text-gray-400 mt-0.5">Äáº·t má»©c giá»›i háº¡n chi tiÃªu Ä‘á»ƒ giá»¯ bÃ³p nhen!</p>
            </div>
            ...
            <div className="text-right">
              {isEditingBudget ? (
                <div className="flex items-center gap-1">
                  ...
                  />
                  <button onClick={handleSaveBudget} className="text-emerald-500 text-xs font-bold">LÆ°u</button>
                  <button onClick={() => setIsEditingBudget(false)} className="text-gray-400 text-xs">Há»§y</button>
                </div>
              ) : (
                <button
                  id="edit-budget-btn"
                ...
                >
                  Sá»­a: Â₫{budget.limit.toLocaleString()}
                </button>
              )}
            </div>
          </div>
          ...
          <div className="flex justify-between items-center text-[11px] text-gray-400 font-medium mt-1.5 px-1">
            <span>ÄÃ£ tiÃªu: Â₫{totalExpense.toLocaleString()}</span>
            <span>CÃ²n láº¡i: Â₫{Math.max(0, budget.limit - totalExpense).toLocaleString()}</span>
          </div>
        </div>
        ...
        <div>
          <h3 className="text-sm font-display font-bold text-kawaii-pink-600 mb-2 px-1 flex items-center gap-1">
            â­ Má»₫c tiÃªu tiáº¿t kiá»‡m dá»… thÆ°Æ¡ng
          </h3>
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-display font-semibold text-kawaii-pink-600 flex items-center gap-1">
                  ðŸŽ¯ Tiáº¿t kiá»‡m cho: {goals.find(g => g.id === selectedGoalId)?.name}
                </span>
              ...
              <form onSubmit={handleGoalContributionSubmit} className="flex gap-2">
                <div className="relative flex-1">
                  <span className="absolute left-2.5 top-1.5 text-xs text-gray-400 font-mono">Â₫</span>
                  <input
                    id="goal-contrib-input"
                    type="number"
                    placeholder="Sá»‘ tiá»n gá»­i..."
        ...
        <div>
          <div className="flex items-center justify-between mb-2 px-1">
            <h3 className="text-sm font-display font-bold text-kawaii-pink-600 flex items-center gap-1.5">
              ðŸŒ¸ Nháº­t KÃ½ Giao Dá»‹ch Gáº§n ÄÃ¢y
            </h3>
            <span className="text-[10px] font-mono text-gray-400">
              {filteredTransactions.length} giao dá»‹ch
            </span>
          </div>
          ...
          {filteredTransactions.length === 0 ? (
            <div className="kawaii-card p-6 bg-white text-center flex flex-col items-center gap-2">
              ...
              <p className="text-xs text-gray-400 font-medium">ChÆ°a cÃ³ giao dá»‹ch nÃ o háº¿t Ä‘Ã³ nhen!</p>
              <button
                onClick={onAddTransactionClick}
                className="kawaii-button kawaii-button-primary text-xs py-1.5 px-4 font-display flex items-center gap-1 mt-1"
              >
                <Plus size={14} /> Ghi chÃ©p ngay
              </button>
            </div>
          ) : (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        id={`tx-delete-${tx.id}`}
                        onClick={() => {
                          if (confirm('Báº¡n cÃ³ cháº¯c muá»‘n xÃ³a giao dá»‹ch siÃªu dá»… thÆ°Æ¡ng nÃ y khÃ´ng? ðŸ₫º')) {
